...
const ProfileAccounts = ({
  ...
  return (
    <>
      {Object.keys(profileAccounts.online).length > 0 && (
        <StyledContainer>
          {/* <StyledLabel>
          ...
          {Object.keys(profileAccounts.online).map(
            (element: string, index: number) => {
              ...
              let accountName: any = ObjectFinder(element)(
                profileAccounts.online
              )
      ...
      {(Object.keys(profileAccounts.crypto.identities).length > 0 ||
        Object.keys(profileAccounts.crypto.addresses).length > 0) && (
        <StyledContainer>
          {/* <StyledLabel>
          <DefaultHeader as="h5" align="left">
            Blockchain Accounts
          ...
          {Object.keys(profileAccounts.crypto.addresses).map(
            (element, index) => {
              let image
              switch (element) {
                case 'vrsc':
